# PowerShell script to import residents from Excel file
# Usage: .\ImportResidents.ps1 -FilePath "C:\Users\Justine\Downloads\Resident Template BLOCK 5.xlsx" -BhwName "Emily rotairo"
...
$apiProjectPath = Join-Path $projectDir "BarangayCIS.API"
...
# Create a simple C# program to run the import
$importScript = @"
using System;
...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        var connectionString = "Server=(localdb)\\mssqllocaldb;Database=BarangayCIS;Trusted_Connection=True;MultipleActiveResultSets=true";
        ...
        for (int col = 1; col <= worksheet.LastColumnUsed().ColumnNumber(); col++)
        {
            ...
            else if (header.Contains("dateofbirth") || header.Contains("date of birth")) columnMap["DateOfBirth"] = col;
...
$tempScript = Join-Path $env:TEMP "ImportResidents.cs"
$importScript | Out-File -FilePath $tempScript -Encoding UTF8

Write-Host "Import script created. Please run this manually using dotnet run or create a console app." -ForegroundColor Yellow
Write-Host "Alternatively, you can use the API endpoint: POST /api/residents/import?filePath=<path>&bhwName=Emily rotairo" -ForegroundColor Yellow
